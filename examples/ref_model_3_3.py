...
variants = {'o': 'original', 'v': 'variant', 'w': 'wariant', 'x': 'xariant', 'y': 'yariant'}
# color rotation
irc = {'os': 1, 'bt': 2, 've': 3, 'ne': 4}

variants_in_cycle = {'os': ['o', 'v', 'w', 'x', 'y'],
                     'bt': ['o', 'v', 'w', 'x', 'y'],
                     've': ['x', 'y'],
                     'ne': ['x', 'y']}
...
# a fraction of all strain infections will be susceptible to natural escape (except xariant and yariant)
# ------------------------------------------------------------------------------------------
...
colors = ['mediumpurple', 'darkslateblue', 'mediumslateblue', 'darkviolet', 'palevioletred']
deaths_pops = {}
deaths_pops_xy = {}
for variant, color in zip(variants, colors):
    if variant in ['x', 'y']:
        for cycle in cycles:
            deaths_pops_xy[cycle] = Population(cycle + ' deaths ' + variant, 0,
            ...
            deaths_pops[variant + cycle] = deaths_pops_xy[cycle]
            ...
            bc_model.add_connector(
                Splitter(cycle + ' recovery ' + variant, contagious_pops[variant][cycle],
                         [recovered_pop, deaths_pops_xy[cycle]],
...
deaths_pop_xy = Population('deaths xy', 0,
...
collector_populations[deaths_pop_xy] = deaths_pops_xy
...
removed_pops = {}
color = ['blue', 'darkslateblue', 'dodgerblue', 'lightskyblue', 'midnightblue']
for variant, color in zip(variants, colors):
...
colors = ['chocolate', 'peru', 'burlywood', 'blanchedalmond', 'wheat']
...
colors = ['springgreen', 'olive', 'tomato', 'slateblue', 'violet']
reported_pops = {}
for variant, color in zip(variants, colors):
...
colors = ['indianred', 'hotpink', 'rosybrown', 'brown', 'darkred']
...
colors = ['deeppink', 'hotpink', 'rosybrown', 'brown', 'peachpuff']
icu_pops = {}
for variant, color in zip(variants, colors):
    for cycle in cycles:
        if variant in variants_in_cycle[cycle]:
...
n_rate_transitions = {'o': 14, 'v': 6, 'w': 12, 'x': 4, 'y': 4}
n_rt_visible = {'o': 4, 'v': 0, 'w': 0, 'x': 0, 'y': 0}
...
n_outbreaks = {'o': 7, 'v': 3, 'w': 4, 'x': 3, 'y': 3}
