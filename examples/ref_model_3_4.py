...
Version 3.0:
 ...
 Version 3.1:
  ...
  - include booster function to reduce vaccine escape (in addition to reducing breakthrough)

Version 3.2:
  - To allow for omicron death rates to be fit, replace cycle-based recovery parameters by single scaling parameter,
    death_scale_x
...
Version 3.4:
  - Include a new strain (labelled z) for Omicron BA.4 and BA.5 (combined)
  - Add separate parameters for waning of immunity (natural and vaccine) for omicron,
    so that pre-omicron fits do not have to be changed
...
variants = {'o': 'original', 'v': 'variant', 'w': 'wariant', 'x': 'xariant', 'y': 'yariant', 'z': 'zariant'}
# color rotation
irc = {'os': 1, 'bt': 2, 've': 3, 'ne': 4}

variants_in_cycle = {'os': ['o', 'v', 'w', 'x', 'y', 'z'],
                     'bt': ['o', 'v', 'w', 'x', 'y', 'z'],
                     've': ['x', 'y', 'z'],
                     'ne': ['x', 'y', 'z']}
...
# infected
colors = ['orange', 'darkgoldenrod', 'sienna', 'sandybrown', 'darkkhaki', 'coral']
...
# contagious
colors = ['red', 'rosybrown', 'maroon', 'lightcoral', 'orangered', 'indianred']
...
# a fraction of all strain infections will be susceptible to natural escape (except xariant, yariant, and 'zariant')
# ------------------------------------------------------------------------------------------
...
# Starting with 3.4: Include waning of natural immunity for those infected through the 'ne' cycle (omicron only)

cycle_list.append('ne')
...
ne_nat_waned_delay_pars = {
    'mean': Parameter('ne_nat_waned_delay_mean', 365., 1., 10000.,
                      'mean time from ne natural immunity to ne waned immunity'),
    'sigma': Parameter('ne_nat_waned_delay_sigma', 50., 1., 500.,
                       'standard deviation of times from ne natural immunity to ne waned immunity')
}
ne_nat_waned_delay = Delay('ne_nat_waned_delay', 'gamma', ne_nat_waned_delay_pars, bc_model)

ne_nat_waned_fraction = Parameter('ne_nat_waned_frac', 1., 0., 1.,
                                  'fraction of ne natural immunizations that wane')
...
for cycle in cycle_list:
    ...
    if cycle != 'ne':
        ...
        bc_model.add_connector(
            Propagator(cycle + ' nat waned immunity', nat_immunized_pops[cycle], waned_nat_immunity_pop,
                       ne_nat_waned_fraction, ne_nat_waned_delay))
...
boost_waned_delay_pars = {
    'mean': Parameter('boost_waned_delay_mean', 365., 1., 10000.,
                      'mean time from booster immunity to waned immunity'),
    'sigma': Parameter('boost_waned_delay_sigma', 50., 1., 500.,
                       'standard deviation of times from booster immunity to waned immunity')
}
boost_waned_delay = Delay('boost_waned_delay', 'gamma', boost_waned_delay_pars, bc_model)

boost_waned_fraction = Parameter('boost_waned_frac', 1., 0., 1.,
                                 'fraction of booster immunizations that wane')
...
bc_model.add_connector(
    Propagator('ve boost waned immunity', ve_removed_pop, waned_boost_ve_removed_pop, boost_waned_fraction,
               boost_waned_delay))
...
death_scale_x = Parameter('death_scale_x', 0.1, 0., 1.,
                          'scale factor for death fraction for xariant', hidden=False)
number_one = Parameter('number_one', 1., 0., 1., '1')
recover_fraction_x = Operator([recover_fraction, death_scale_x, death_scale_x, number_one], '*-+')
...
colors = ['mediumpurple', 'darkslateblue', 'mediumslateblue', 'darkviolet', 'palevioletred', 'orchid']
deaths_pops = {}
deaths_pops_xyz = {}
for variant, color in zip(variants, colors):
    if variant in ['x', 'y', 'z']:
        for cycle in cycles:
            deaths_pops_xyz[cycle] = Population(cycle + ' deaths ' + variant, 0,
            ...
            deaths_pops[variant + cycle] = deaths_pops_xyz[cycle]

            # the following (from model 3.1 is replaced by a single scaling parameter)
            # if necessary to consider scenarios of different severity for ve etc, can scale the death populations
            ...
            bc_model.add_connector(
                Splitter(cycle + ' recovery ' + variant, contagious_pops[variant][cycle],
                         [recovered_pop, deaths_pops_xyz[cycle]],
...
deaths_pop_xyz = Population('deaths xy', 0,
                            'all those who have died from xy', hidden=True,
                            color=color, show_sim=True,
                            report_noise=True, report_noise_par=death_noise_par,
                            report_backlog_par=death_backlog_par, report_days=death_report_days,
                            report_noise_weekly=True
                            )
collector_populations[deaths_pop_xyz] = deaths_pops_xyz
...
removed_pops = {}
color = ['blue', 'darkslateblue', 'dodgerblue', 'lightskyblue', 'midnightblue', 'cadetblue']
for variant, color in zip(variants, colors):
...
colors = ['chocolate', 'peru', 'burlywood', 'blanchedalmond', 'wheat', 'goldenrod']
...
colors = ['springgreen', 'olive', 'tomato', 'slateblue', 'violet', 'fuchsia']
reported_pops = {}
for variant, color in zip(variants, colors):
...
colors = ['indianred', 'hotpink', 'rosybrown', 'brown', 'darkred', 'orangered']
...
colors = ['deeppink', 'hotpink', 'rosybrown', 'brown', 'peachpuff', 'bisque']
icu_pops = {}
for variant, color in zip(variants, colors):
    for cycle in cycles:
        if variant in variants_in_cycle[cycle]:
...
n_rate_transitions = {'o': 14, 'v': 6, 'w': 12, 'x': 4, 'y': 4, 'z': 4}
n_rt_visible = {'o': 4, 'v': 0, 'w': 0, 'x': 0, 'y': 0, 'z': 0}
...
n_outbreaks = {'o': 7, 'v': 3, 'w': 4, 'x': 3, 'y': 3, 'z': 3}
